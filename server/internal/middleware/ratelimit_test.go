...
func TestExtractIP_IPv6Normalization(t *testing.T) {
	req1 := httptest.NewRequest(http.MethodPost, "/auth/google", nil)
	req1.RemoteAddr = "[::1]:9000"

	req2 := httptest.NewRequest(http.MethodPost, "/auth/google", nil)
...
func TestRateLimit_LuaScript_SetsTTL(t *testing.T) {
	...
	key := rateLimitKey("/auth/google", "10.0.99.1")
