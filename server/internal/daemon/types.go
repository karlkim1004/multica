...
type Task struct {
	...
	// ChatHistory is a provider-neutral transcript of this chat session's
	// prior turns, populated only when PriorSessionID is empty (no native
	// session to resume — e.g. the runtime changed since the last turn,
	// NEX-964). Without it, a runtime switch silently drops the whole
	// conversation because ChatMessage only carries the latest unanswered
	// message(s). Capped in length by the server; see buildChatHistoryText.
