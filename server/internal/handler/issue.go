...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if shouldRunDoneEvidenceGate(prevIssue, req.Status) {
		if status, msg := h.checkDoneEvidenceGate(r.Context(), prevIssue, doneTransitionCallsNextAgent(prevIssue, req.AssigneeType, req.AssigneeID)); status != 0 {
			writeError(w, status, msg)
			return
		}
	}
	...
	if req.Updates.Status != nil && *req.Updates.Status == "done" {
		for _, issueID := range req.IssueIDs {
			...
			if shouldRunDoneEvidenceGate(prevIssue, req.Updates.Status) {
				if status, msg := h.checkDoneEvidenceGate(r.Context(), prevIssue, doneTransitionCallsNextAgent(prevIssue, req.Updates.AssigneeType, req.Updates.AssigneeID)); status != 0 {
					writeError(w, status, msg)
					return
				}
			}
