...
const countOpenPullRequestsByIssue = `-- name: CountOpenPullRequestsByIssue :one
SELECT COUNT(*)::bigint FROM github_pull_request pr
JOIN issue_pull_request ipr ON ipr.pull_request_id = pr.id
WHERE ipr.issue_id = $1 AND pr.state IN ('open', 'draft')
`

func (q *Queries) CountOpenPullRequestsByIssue(ctx context.Context, issueID pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countOpenPullRequestsByIssue, issueID)
	var column_1 int64
