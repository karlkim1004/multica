...
const autoCloseIssueAfterValidation = `-- name: AutoCloseIssueAfterValidation :one
UPDATE issue AS i
...
RETURNING i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.auto_close_allowed, i.implementation_agent_id, i.current_ref, i.external_validation_required, i.auto_close_criteria_version
`

type AutoCloseIssueAfterValidationParams struct {
...
func (q *Queries) AutoCloseIssueAfterValidation(ctx context.Context, arg AutoCloseIssueAfterValidationParams) (Issue, error) {
	row := q.db.QueryRow(ctx, autoCloseIssueAfterValidation, arg.IssueID, arg.WorkspaceID, arg.CommentID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const countOpenChildIssues = `-- name: CountOpenChildIssues :one
SELECT COUNT(*)::bigint FROM issue
WHERE parent_issue_id = $1 AND workspace_id = $2
  AND status NOT IN ('done', 'cancelled')
`

type CountOpenChildIssuesParams struct {
	ParentIssueID pgtype.UUID `json:"parent_issue_id"`
	WorkspaceID   pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) CountOpenChildIssues(ctx context.Context, arg CountOpenChildIssuesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countOpenChildIssues, arg.ParentIssueID, arg.WorkspaceID)
	var column_1 int64
...
type ListIssuesRow struct {
	...
	ImplementationAgentID      pgtype.UUID        `json:"implementation_agent_id"`
	CurrentRef                 pgtype.Text        `json:"current_ref"`
	ExternalValidationRequired bool               `json:"external_validation_required"`
	AutoCloseCriteriaVersion   pgtype.Text        `json:"auto_close_criteria_version"`
}
...
UPDATE issue SET
    ...
    project_id = $12,
    stage = $13,
    auto_close_allowed = COALESCE($14, auto_close_allowed),
    implementation_agent_id = $15,
    current_ref = $16,
    external_validation_required = COALESCE($17, external_validation_required),
    auto_close_criteria_version = $18,
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, auto_close_allowed, implementation_agent_id, current_ref, external_validation_required, auto_close_criteria_version
`
...
type UpdateIssueParams struct {
	...
	ProjectID                  pgtype.UUID   `json:"project_id"`
	Stage                      pgtype.Int4   `json:"stage"`
	AutoCloseAllowed           pgtype.Bool   `json:"auto_close_allowed"`
	ImplementationAgentID      pgtype.UUID   `json:"implementation_agent_id"`
	CurrentRef                 pgtype.Text   `json:"current_ref"`
	ExternalValidationRequired pgtype.Bool   `json:"external_validation_required"`
	AutoCloseCriteriaVersion   pgtype.Text   `json:"auto_close_criteria_version"`
}
...
func (q *Queries) UpdateIssue(ctx context.Context, arg UpdateIssueParams) (Issue, error) {
	row := q.db.QueryRow(ctx, updateIssue,
		...
		arg.ProjectID,
		arg.Stage,
		arg.AutoCloseAllowed,
		arg.ImplementationAgentID,
		arg.CurrentRef,
		arg.ExternalValidationRequired,
		arg.AutoCloseCriteriaVersion,
	)
	var i Issue
	err := row.Scan(
		&i.ID,
