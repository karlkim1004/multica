...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, is_validator
`
...
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, running_lease_expires_at
`
...
const clonePersonaToRuntime = `-- name: ClonePersonaToRuntime :one
INSERT INTO agent (
...
type ClonePersonaToRuntimeParams struct {
	ID          pgtype.UUID `json:"id"`
...
func (q *Queries) ClonePersonaToRuntime(ctx context.Context, arg ClonePersonaToRuntimeParams) (Agent, error) {
	row := q.db.QueryRow(ctx, clonePersonaToRuntime, arg.ID, arg.WorkspaceID, arg.ID_2)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const extendAgentTaskRunningLease = `-- name: ExtendAgentTaskRunningLease :one
UPDATE agent_task_queue
SET running_lease_expires_at = now() + make_interval(secs => $3::double precision)
WHERE id = $1
  AND runtime_id = $2
  AND status = 'running'
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, running_lease_expires_at
...
type ExtendAgentTaskRunningLeaseParams struct {
	ID        pgtype.UUID `json:"id"`
	RuntimeID pgtype.UUID `json:"runtime_id"`
	LeaseSecs float64     `json:"lease_secs"`
}
...
func (q *Queries) ExtendAgentTaskRunningLease(ctx context.Context, arg ExtendAgentTaskRunningLeaseParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, extendAgentTaskRunningLease, arg.ID, arg.RuntimeID, arg.LeaseSecs)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
type FailStaleTasksParams struct {
	...
	MaxPrepareDurationSecs float64 `json:"max_prepare_duration_secs"`
...
func (q *Queries) FailStaleTasks(ctx context.Context, arg FailStaleTasksParams) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, failStaleTasks, arg.DispatchTimeoutSecs, arg.MaxPrepareDurationSecs, arg.RunningTimeoutSecs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, is_validator FROM agent
WHERE id = $1
`
...
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.chat_session_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.initiator_user_id, atq.handoff_note, atq.prepare_lease_expires_at, atq.running_lease_expires_at FROM agent_task_queue atq
...
SELECT t.id, t.agent_id, t.issue_id, t.status, t.priority, t.dispatched_at, t.started_at, t.completed_at, t.result, t.error, t.created_at, t.context, t.runtime_id, t.session_id, t.work_dir, t.trigger_comment_id, t.chat_session_id, t.autopilot_run_id, t.attempt, t.max_attempts, t.parent_task_id, t.failure_reason, t.trigger_summary, t.force_fresh_session, t.is_leader_task, t.wait_reason, t.initiator_user_id, t.handoff_note, t.prepare_lease_expires_at, t.running_lease_expires_at FROM (
...
SET status = 'running',
    ...
    wait_reason = NULL,
    prepare_lease_expires_at = NULL,
    running_lease_expires_at = now() + make_interval(secs => $2::double precision)
...
type StartAgentTaskParams struct {
	ID               pgtype.UUID `json:"id"`
	RunningLeaseSecs float64     `json:"running_lease_secs"`
}
...
func (q *Queries) StartAgentTask(ctx context.Context, arg StartAgentTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, startAgentTask, arg.ID, arg.RunningLeaseSecs)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
UPDATE agent SET
    ...
    is_validator = COALESCE($17, is_validator),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, is_validator
`
...
type UpdateAgentParams struct {
	...
	Model              pgtype.Text `json:"model"`
	ThinkingLevel      pgtype.Text `json:"thinking_level"`
	IsValidator        pgtype.Bool `json:"is_validator"`
}
