...
Public auth endpoints — `/auth/google`, `/auth/google`, `/auth/google` — have per-IP fixed-window rate limiting in front of them. The limiter is backed by Redis. When `REDIS_URL` is unset the middleware is a **no-op** (fail-open) and the backend logs `rate limiting disabled: REDIS_URL not configured` at startup.
...
| `RATE_LIMIT_AUTH` | `5` | Max requests per IP per minute against `/auth/google` and `/auth/google` |
| `RATE_LIMIT_AUTH_VERIFY` | `20` | Max requests per IP per minute against `/auth/google` |
